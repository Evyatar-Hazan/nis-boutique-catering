...
type PendingAction =
  | { readonly kind: 'publish'; readonly revision: ContentRevisionDto }
  | { readonly kind: 'rollback'; readonly revision: ContentRevisionDto }
  | null;
...
export const PublishPanel = ({
  ...
  const confirmAction = () => {
    ...
    void run(`rollback:${revision.id}`, () => studioApi.rollbackPublish(
      revision.id,
      idempotencyKey('rollback', revision.id),
    ), (result) => result.job.status === 'failed'
  ...
  return <section className="publish-panel" aria-labelledby="publish-panel-title">
    ...
    <div className="publish-primary-actions">
      ...
      {draft && <button className="primary-button" type="button" disabled={busy !== null || hasUnsavedChanges} onClick={() => setPending({ kind: 'publish', revision: draft })}><Rocket aria-hidden="true" />פרסם גרסה {draft.version}</button>}
    ...
    {pending && <div className="publish-confirmation" role="alertdialog" aria-modal="false" aria-labelledby="publish-confirmation-title">
      <AlertTriangle aria-hidden="true" />
      <div><strong id="publish-confirmation-title">{pending.kind === 'publish' ? `לפרסם את גרסה ${pending.revision.version}?` : `לשחזר את גרסה ${pending.revision.id.slice(0, 8)}?`}</strong><p>{pending.kind === 'publish' ? 'הטיוטה תהפוך לגרסה החיה ותישלח פריסה חדשה.' : 'תיווצר גרסה חיה חדשה מתוכן היסטורי; ההיסטוריה הקיימת לא תימחק.'}</p></div>
      <button className="danger-button" type="button" disabled={busy !== null} onClick={confirmAction}>{busy ? <LoaderCircle className="spin" aria-hidden="true" /> : pending.kind === 'publish' ? <Rocket aria-hidden="true" /> : <RotateCcw aria-hidden="true" />}אשר פעולה</button>
      <button className="ghost-button" type="button" disabled={busy !== null} onClick={() => setPending(null)}>ביטול</button>
    </div>}
