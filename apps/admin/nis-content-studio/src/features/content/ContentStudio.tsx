import { publicSiteDocumentSchema, type PublicSiteDocument } from '@monorepo/content-schema';
import { PublicSiteDocumentPreview } from '@monorepo/site-preview';
import { AlertTriangle, Check, Eye, RefreshCw, Save } from 'lucide-react';
import { useEffect, useMemo, useState } from 'react';

import { StudioApiError } from '../../api/client';
import { studioApi, type ContentRevisionDto } from '../../api/studioApi';
import {
  buildEditorGroups,
  editableSectionIds,
  readEditorValue,
...
const FieldControl = ({
  ...
  return <label className={field.kind === 'checkbox' ? 'toggle-field' : 'field'} htmlFor={id}>
    ...
    {field.kind === 'checkbox' && <input {...common} checked={Boolean(value)} type="checkbox" onChange={(event) => onChange(event.currentTarget.checked)} />}
  ...
  return <section className="content-studio" aria-labelledby="content-studio-title">
    ...
    <div className="content-workspace">
      ...
      <aside className="content-preview" aria-labelledby="content-preview-title">
        ...
        <PublicSiteDocumentPreview document={content} />
      </aside>
    </div>
  </section>;
};
